...
Drawing MachineAssemblyProduction = StandardAssemblyDrawing<Product: Machine, Metadata: MachineDrawingInfo>`,
      ...
      {
        kind: "figure",
        src: "/aetheris/assets/preview2/bearing-block-review.png",
        alt: "Editable A4 bearing block review drawing with an orange semantic DFM callout attached to bore PMI",
        caption:
          "Review mode adds native editable callout, leader, highlight, identity, status, author, and authored date while keeping the production deck clean.",
      },
      {
        kind: "figure",
        src: "/aetheris/assets/preview2/bearing-block-dfm-review.png",
        alt: "16 by 9 DFM proposal slide with a large projected bearing block view, highlighted bore, arrow, and current versus proposed tolerance",
        caption:
          "The presentation-oriented DFM deck uses the same engineering target and DrawingIR view, with a practical big-arrow layout and structured current/proposed values.",
      },
      html(
        '<h2>Editable PowerPoint is now a sibling backend</h2><p>The same DrawingIR now lowers offline to exact-A4 PowerPoint slides made from native lines, dashed hidden edges, editable Inter text, semantic groups, metadata, zones, Design Tables, and BOM tables. Production decks stay clean. Structured Firmament Review threads can project editable callouts onto real PMI, and a separate 16:9 DFM deck generates practical highlighted issue/proposal slides.</p><div class="callout"><strong>PowerPoint is an editor for the compiled drawing artifact, not the authoritative CAD model.</strong><p>Manual slide edits never mutate the Firmament product or PMI. Reviews remain structured authored source with stable semantic targets, authors, dates, typed status, and current/proposed values.</p></div><h2>Bounded M0 contract</h2><ul><li>A4 portrait or landscape with stable A-D / 1-6 semantic zones;</li><li>manual orthographic/isometric views and manual PMI assignment;</li><li>occurrence-aware nested AssemblyIR projection without Boolean flattening;</li><li>VisibleOnly/VisibleAndHidden interval classification with split and unsupported-support evidence;</li><li>flattened leaf-part BOM from AssemblyIR;</li><li>typed Version/Date DrawingInfo with Static/with provenance;</li><li>searchable native PDF with embedded Inter and no raster pages;</li><li>native editable PPTX with deterministic semantic names and no full-page raster;</li><li>semantic Issue, Proposal, and Resolution data plus A4 Review and 16:9 DFM projections.</li></ul><p>The HLR oracle is deliberately bounded by admitted face tessellation; unsupported patches are explicit and conservative. PowerPoint font embedding, analytic arc records, redline import, Graph delivery, sections/details, exploded views, and complete ISO/ASME coverage remain future work.</p>',
      ),
    ],
